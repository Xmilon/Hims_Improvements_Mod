...
public abstract class LivingEntity extends Entity implements Attackable {
	...
	protected int computeFallDamage(float fallDistance, float damageMultiplier) {
		if (this.getType().isIn(EntityTypeTags.FALL_DAMAGE_IMMUNE)) {
			...
			return MathHelper.ceil(g * damageMultiplier * this.getAttributeValue(EntityAttributes.GENERIC_FALL_DAMAGE_MULTIPLIER));
		}
	}
	...
	private boolean shouldSpawnConsumptionEffects() {
		...
		int j = (int)(this.activeItemStack.getMaxUseTime(this) * 0.21875F);
		boolean bl = i > j;
	...
	protected void spawnConsumptionEffects(ItemStack stack, int particleCount) {
		if (!stack.isEmpty() && this.isUsingItem()) {
			...
			if (stack.getUseAction() == UseAction.EAT) {
				...
				this.playSound(this.getEatSound(stack), 0.5F + 0.5F * this.random.nextInt(2), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
			}
		}
	}
	...
	public void setOnFireForTicks(int ticks) {
		super.setOnFireForTicks(MathHelper.ceil(ticks * this.getAttributeValue(EntityAttributes.GENERIC_BURNING_TIME)));
	}
	...
	public boolean isInvulnerableTo(DamageSource damageSource) {
		return super.isInvulnerableTo(damageSource)
			|| this.getWorld() instanceof ServerWorld serverWorld && EnchantmentHelper.isInvulnerableTo(serverWorld, this, damageSource);
	}
